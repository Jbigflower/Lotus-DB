...
onMounted(async () => {
  ...
  try {
    ...
    const target = (res.items ?? []).find((c: any) => c.type === CustomListType.WATCHLIST && (!uid || c.user_id === uid))
    const id = target?.id
    if (id) { currentListId.value = id; await collStore.fetchMovies(token, id) }
    else { ElMessage.error('未找到当前用户的待观看片单') }
  } catch { ElMessage.error('加载待观看片单失败') }
})
</script>

<template>
  <!-- 稍后看：克隆布局结构，包含筛选与完整排序交互 -->
  <section class="watch-later content mode-media" :data-route="route.path">
    <ListToolbar
      :total="total"
    ...
    <div v-if="showFilter" class="panel panel--filter">
      ...
      <p class="hint">本页演示为“稍后看”片单筛选，交互与库详情保持一致。</p>
    </div>
    ...
    <MediaGrid
      :items="paged"
      :dense="viewMode==='list'"
      :projector="(raw: any) => { const id=String(raw?.id ?? ''); const ent=movieStore.entities[id]; return { id, title: raw.title, poster: raw.poster, rating: raw.rating, tags: raw.tags, isFavorite: (raw?.is_favoriter===true) || (ent?.is_favoriter===true), inWatchLater: (raw?.is_watchLater===true) || (ent?.is_watchLater===true) } }"
      @open="openMovie"
      @play="openPlayer"
      @preview="(id) => void 0"
...
.watch-later { display: grid; gap: var(--space-4); }
